...
@section('page_name') {{ __('lang.Knowledge creation') }} @endsection
...
@section('content')
    ...
    <section class="news-section" id="news">
      <div class="si">
        <div class="news-hdr">
          <div>
            <div class="stag"><span class="stag-line"></span><span>{{ __('lang.Knowledge creation') }}</span></div>
          </div>
        </div>
            <div class="news">
                @forelse ($knowledges as $knowledge)
                      <div class="ncard" style="margin: 10px; background: white; height: 100%;">
                        <div class="nimg" style="background: url(../storage/{{str_replace("\\" , "/",$knowledge->image)}});background-size: cover; background-position: center; height: 230px;">
                        </div>
                        <div class="nbody" style="padding: 1.5rem;">
                          <div class="ndate" style="font-size: 12px; color: var(--muted); margin-bottom: 0.5rem;">{{ app()->getLocale() == 'ar' ? to_arabic_number(date('F Y', strtotime($knowledge->created_at))) : $knowledge->created_at->formatLocalized('%B %Y') }}</div>
                          <h4 style="font-size: 16px; font-weight: 700; margin-bottom: 1rem; line-height: 1.4; min-height: 48px; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">{{$knowledge->$title}}</h4>
                          <a href="/pages/knowledges/{{$knowledge->id}}" class="nread" style="font-size: 13px; color: var(--teal); font-weight: bold; text-decoration: none; display: inline-flex; align-items: center; gap: 5px;">{{ __('lang.more') }} {{ __('lang.arrow_dir') }}</a>
                        </div>
                      </div>
                @empty
                @endforelse
            </div>
            <div class="pagination" >{!! $knowledges->links() !!}</div>
      </div>
    </section>
